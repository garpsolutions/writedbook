
<div id="form-estudiantes" style="background-color: white; margin-top:50px; width:60%; padding:20px; margin-left:24%;">
    <div>
    <form action="" method="post">
      <h5> Documentos</h5> 
        <div class="row">
            <div class="col-md-6"><br>
                <label for="">Fotografia</label>
                <input class="form-control" type="file" name="" placeholder="Nombre de un familiar cercano al padre" id="" require>
            </div>
            <div class="col-md-6"><br>
                <label for="">Acta de nacimiento</label>
                <input class="form-control" type="file" placeholder="Parentesco" name="" id="" require>
            </div>
            <div class="col-md-6"><br>
                <label for="">Copia cedula de la madre</label>
                <input class="form-control" type="file" placeholder="Direccion" name="" id="" require>
            </div>
            <div class="col-md-6"><br>
            <label for="">Copia de cedula padre</label>
                <input class="form-control" type="file" placeholder="000-000-0000" name="" id="" require>
            </div>
            <div class="col-md-6"><br>
                <a   class="btn btn-primary " id="anterior4">Anterior</a>
            </div>
            <div class="col-md-6"><br>
                <a  style="float: right;"
                class="btn btn-danger " id="siguiente4">Guardar</a>
            </div>
        </form>
        </div>
    </div>
</div>